...
Validate a future sanitized M2 live-evidence bundle without contacting EDA:
...
The gate keeps M2 pending on missing evidence, rejects hash/privacy failures and
writes only one minimal idempotent summary after the complete BEFORE/AFTER loop
passes. The bundled positive gate fixture is synthetic and carries no live claim.
...
This alpha primarily publishes the review model, evidence structure, sanitized evaluations and bounded workflow. Automatic repair is described only at the status shown in [supported-repairs.md](docs/supported-repairs.md). The M2 BEFORE/AFTER pair remains pending live EDA and save/reload verification; its passing AFTER result is an offline forecast, not a completed live repair claim. See [Limitations](docs/limitations.md), [Roadmap](docs/roadmap.md), and fixture-scoped [resume wording](docs/resume.md) for claim boundaries.
